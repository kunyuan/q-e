...
SUBROUTINE c_bands( iter )
  ...
  USE uspp,                 ONLY : vkb, nkb ! total number of beta functions, with struct.fact.
  USE gvect,                ONLY : g
  ...
  k_loop: DO ik = ik_+1, nks
     ...
     !
     CALL g2_kin( ik ) ! kinetic energy
     !
     ! ... More stuff needed by the hamiltonian: nonlocal projectors (for ultra soft pseudos)
     !
